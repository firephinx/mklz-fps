...
fun sortRange (array, start, n, compare) = 
  let
      ...
      (* using unsafe subscripting, makes about 10% difference *)
      ...
      fun qsort(a, n) = 
        ...
        in if (n <= 4000) then 
               (qsort(a, l-a);                      
                ...
                qsort(m, a+n-m))
           else let val par = Primitives.par
...
fun sortInplace compare aslice = 
  let val (a, i, n) = ArraySlice.base aslice
  in sortRange(a, i, n, compare) end

(* sorts an sequence without modifying input *)
fun sort compare Ar = 
  sortInplace compare (ArraySequence.map (fn x => x) Ar)
