...
struct
  ...
  struct
    ...
    fun mapReduce (f : 'a -> 'b) (g : 'b * 'b -> 'b) (b : 'b) (s : 'a seq) : 'b =
      let
        ...
        fun mapReduceSeq(s,b,e) =
	  if (e-b = 1) then f (AS.sub(s, b))
          else let val mid = Word.toInt(Word.>>(Word.fromInt(e+b),
                                                Word.fromInt(1)))     (* (e+b) div 2 *)
                   in g (mapReduceSeq(s,b,mid), mapReduceSeq(s,mid,e))
                   end
        ...
        fun mapReduce' s =
            ...
            | PAIR (l, r) => 
                if length s <= k
                then mapReduceSeq(s, 0, length s)
 ...
 fun reduce (g : 'b * 'b -> 'b) (b : 'b) (s : 'b seq) : 'b =
      let
        val k = G.get ()
        fun mapReduce' s =
          if length s <= k
          then (iterate g b s)
            ...
            case splitMid s of
              EMPTY => b
            | ONE x => x
            | PAIR (l, r) => g (Primitives.par (fn () => mapReduce' l,
                                                fn () => mapReduce' r))
      in
        mapReduce' s
      end
