...
func just_sort_blocks(in []Pair, n_threads int) {
  fmt.Printf("Just sort!\n")
  ...
  for i:=0;i<n_threads;i++{
    ...
    go sequential_sort(in[start:end], done_channel)
  }
...
func main() {
  ...
  time_begin_jsb := time.Now()
  just_sort_blocks(input, n_threads)
  elapsed_jsb := time.Since(time_begin_jsb)


  fmt.Printf("Time taken to just sort blocks: %s\n", elapsed_jsb)
  // Sort the sequence!
